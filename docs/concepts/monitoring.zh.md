...
### 摘要如何产生——可选启用的 agent-CLI 观察者

滚动摘要由一次**可选启用的、超轻量 one-shot agent-CLI 调用**产生,而非长驻的模型会话。它完全经
`.yanshi.toml` 的 `[summarizer]` 配置,且**默认关闭**(`enabled = false`):

- `enabled`——总开关;为 `false` 时,摘要*始终*为确定性兜底。
- `cli`——由哪个适配器跑摘要(必须在 `[adapters].enabled` 内)。
- `model`——便宜档模型(haiku / flash / mini 类),让观察者近乎零成本。
- 节流 / 预算旋钮——`debounce_s`、`min_new_events`、`max_tokens`、`watcher_token_ceiling` 与
  `timeout_s`,约束它运行的频率与开销。

触发时,YanShi 把一份紧凑的事件摘要通过单次**只读**、非监控的 CLI 调用送出,并在监控事件循环
之外执行,因此保持**非阻塞**。该观察者是**单飞**的(同一时刻仅一个摘要,由去抖窗口把关),且
**有界**(每次调用的 `timeout_s` 加上累计的 `watcher_token_ceiling`);它不派生任何被监控的
dispatch,不写入 agent 状态,也绝不递归地摘要自身。

最关键的是,它**始终降级为确定性的事件拼接兜底**:CLI 缺失或未鉴权、超时、reply 无法解析,或
预算耗尽,都会各自记为一条 warning,转而拼接最近的几个重要事件。因此失败的观察者**绝不会阻塞
或中断被监控的运行**。
